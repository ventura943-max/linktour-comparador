...
type Ejercicio = {
  ...
  precio_km: number                            // €/km para el ajuste automático de autonomía
...
export default function ValorCliente({ models, categories, features, values, lang, readOnly = false }: any) {
  ...
  function cargar(row: any) {
    setEj({
      ...
      feature_ids: row.feature_ids ?? null, precio_km: Number(row.precio_km) || 16,
  ...
  // Ajuste de una celda: la autonomía se calcula sola ((km ref − km comp) × €/km); el resto es manual
  function ajusteDe(compId: string, f: any): number {
    if (autonomiaFeat && f.id === autonomiaFeat.id && refModel) {
      const k1 = parseNum(getVal(f.id, refModel.id)); const k2 = parseNum(getVal(f.id, compId))
  ...
  function resetAjustes(compId?: string) {
    ...
    const msg = comp
      ? `¿Poner a 0 TODOS los ajustes de ${`${comp.brand} ${comp.name} ${comp.version || ''}`.trim()} en este ejercicio? Esta acción no se puede deshacer.`
      : `¿Poner a 0 TODOS los ajustes manuales de este ejercicio (${compModels.length} competidores)? Esta acción no se puede deshacer.\nLos MSRP y la autonomía no se tocan.`
