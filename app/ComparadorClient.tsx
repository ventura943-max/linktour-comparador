...
import ValorCliente from './ValorCliente'
import { parseBrand, compareModels } from '@/lib/brand'
...
function Modelos({ t }: { t: T }) {
  ...
  // Agrupación por marca (sin el prefijo de mercado): LINKTOUR y [IT] LINKTOUR van juntos
  const grupos = useMemo(() => {
    const map = new Map<string, any[]>()
    ;[...models].sort(compareModels).forEach(m => {
      const key = parseBrand(m.brand).brand.toUpperCase()
    ...
    <div>
      ...
      <div className="space-y-5">
        {grupos.map(([marca, lista]) => (
          <div key={marca} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-5 py-2.5 bg-[#081224] flex items-center justify-between">
              <span className="text-xs font-black tracking-[.2em] text-white uppercase">{marca}</span>
              <span className="text-[11px] text-slate-400">{lista.length}</span>
            </div>
            <div className="divide-y divide-slate-50">
              {lista.map((m: any) => {
                idx += 1
                const { marketLabel } = parseBrand(m.brand)
                return (
                  <a key={m.id} href={`/modelo/${m.id}`} className="flex items-center justify-between px-5 py-3 hover:bg-blue-50/40 transition group">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="w-5 text-xs font-bold text-slate-300">{idx}</div>
                      ...
                      <div className="min-w-0">
                        <div className="font-bold text-sm group-hover:text-blue-700 truncate">{m.name} {m.version && <span className="text-slate-400 font-normal">{m.version}</span>}</div>
                        <div className="text-xs text-slate-400 flex items-center gap-2">
                          {m.price && <span>{m.price}</span>}
                          {marketLabel && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">{marketLabel}</span>}
                        </div>
                      </div>
                    </div>
        ...
        {models.length === 0 && <div className="text-center py-12 text-slate-400 bg-white rounded-2xl border border-slate-100"><div className="text-4xl mb-3">🚗</div><div className="font-bold">{t.sinVehiculos}</div></div>}
      </div>
    </div>
  )
}
...
export default function ComparadorClient({ models, categories: rawCategories, features: rawFeatures, values }: any) {
  ...
  // El idioma se recuerda en el navegador y lo comparte la ficha de catálogo
  useEffect(() => { try { const l = localStorage.getItem('liux_lang'); if (l === 'es' || l === 'en' || l === 'it') setLangState(l) } catch {} }, [])
  const setLang = (l: Lang) => { setLangState(l); try { localStorage.setItem('liux_lang', l) } catch {} }
